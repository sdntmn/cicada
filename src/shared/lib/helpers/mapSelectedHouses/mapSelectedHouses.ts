import { PremisesItem } from "@/entities/Premises"

import type { PremisesOption } from "../../types/types"

export const mapSelectedHouses = (houses: PremisesItem[], selectedIds: string[]): PremisesOption[] => {
  const houseMap = new Map(houses.map((h) => [h.id, h]))
  return selectedIds
    .map((id) => {
      const house = houseMap.get(id)
      return house ? { id, name: `${house.street}, ${house.house}` } : null
    })
    .filter(Boolean) as PremisesOption[]
}
