...
export interface ColumnConfig<T, C extends string, V extends string = string> {
  ...
  // Реальные колонки (из данных)
  columns: Record<C, DataColumn<T>>

  // Видимые по умолчанию
  defaultVisible: (C | V)[]

  // Порядок отображения (включает и data, и virtual ключи)
  displayOrder: (C | V)[]

  // Обязательные колонки
  requiredColumns: Set<C | V>

  // Виртуальные колонки (вычисляемые)
  virtualColumns: Record<V, VirtualColumn<T>>
}
